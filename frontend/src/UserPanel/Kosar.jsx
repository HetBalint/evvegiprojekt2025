                ...
                <div className="cart-items">
                    {cartItems.map(product => (
                        <div key={product.termekID} className="cart-item">
                            <img src={`http://localhost:8081/kepek/${product.termekKep}`} alt={product.termekNev} className="cart-item-img" />
                            <div className="cart-item-details">
                                ...
                                <div className="quantity-controls">
                                    <button className="dbcsokkento" onClick={() => mennyisegcsokkentes(product.termekID)}>-</button>
                                    <span className="dbszam">{product.dbszam}</span>
                                    <button className="dbnovelo" onClick={() => mennyisegnoveles(product.termekID)}>+</button>
                                    <button className="torles" onClick={() => handleDelete(product.termekID)}>Termék törlése</button>
                                </div>
                    ...
                    <button className="checkout-btn">Tovább a megrendeléshez</button>
                </div>
            )}
        </div>
    );
